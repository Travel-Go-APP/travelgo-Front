...
class _NewUserState extends State<NewUser> {
  ...
  checkfill() {
    // 닉네임 필드가 채워져 있는지 검사
    fillForm = nicknameController.text.isNotEmpty && nicknameController.text.length >= 2;
    if (!fillForm) {
      Errorname = '';
  ...
  checkvalidator() {
    // 1차 유효성 검사
    if (nicknameController.text.contains(RegExp(r'[^\uAC00-\uD7A3A-Za-z0-9\s]', unicode: true))) {
                ...
                Container(
                  ...
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(
                        ...
                        contentPadding: const EdgeInsets.all(5), // 텍스트 가려지는것을 방지?
                        suffixIcon: GestureDetector(
                            onTap: fillForm // 텍스트 필드 지우기
                                ...
                                : null,
                            child: Icon(Icons.cancel, color: fillForm ? showButton : hideButton))),
                    onChanged: (value) {
                      // 텍스트가 입력될때마다 검사
                      setState(() {
                        checkfill(); // 필드 검사
                        checkvalidator(); // 유효성 검사
                      });
                    },
                  ),
                ),
  ...
  Future<void> returnAlert(String name) {
    // 닉네임 검사창
    return showDialog(
